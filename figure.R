...
#Datafile : BANK RETURN AND MACRO CAPM.csv, idio_part.csv, total_in_and_out_idio_424.csv, total_network_scale_424
 ...
 # Fig.2. four Global systemic important banks
  ...
  png("Fig.2.The return of G-SIBs.png", units="in", width=35, height=18, res=300);
   ...
   xtick = c(1,87,198,316,424,547,655,777,893,1016,1131,1254,1370,1492,1609,1732,1844,1965,2057);
...
# fig.3
  ...
  png("Fig3.R-Square.png", units="in", width=30, height=15, res=300);
   ...
   barplot(rev(Rs[,3]),horiz=T,xlim=c(-0.2,0.8),ylim=c(1,30.5),axes=F,col="royalblue3",
             border=NA,space=0.6)
...
# fig.5 network density
  ...
  png("Fig.5 netden_idio.png", units="in", width=35, height=18, res=300);
   ...
   lines(smooth.spline(tn424, spar = 0.8),lty=1, lwd = 7, col="black",xaxt="n");
...
# fig.6. boxplot
  ...
  str(outin);
# thefirst 20 cols are in and then out
...
# in order to compare the different rolling window size, we scale the total network degree first
   ...
   str(data);
   nam  = colnames(data);
   dt   = as.Date(data[, 1], format = "%Y/%m/%d")[1:nrow(data)]
  ...
  png("Fig8.all_network_density.png", units="in", width=35, height=18, res=300);
   ...
   lines(smooth.spline(tol115, spar = 0.8),lty=2, lwd = 7, col="green",xaxt="n");
   xtick = c(116,198,316,424,547,655,777,893,1016,1131,1254,1370,1492,1609,1732,1844,1965)-rep(115,17);
