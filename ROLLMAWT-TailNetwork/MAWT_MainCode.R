...
#See Also : myFUN_rq_lasso,myFUN_roll_lasso, rq_ws_lasso,rq_lasso,roll_ws_lasso, roll_lasso, MAWT.mean, vare,Netden
...
# Step.1. Decompose return into common and idiosyncratic parts
 ...
 #2.3. solve the optimal rolling window size
  ...
  OptimalRoll = MAWT.mean(widths = widths,X = X,Z = Z,k = k,s = s,ww = ww,tau = tau,lambda = lambda,nfirm = cpd,NP = NP,nz=nz)
  OptimalRoll;
  ...
  data = read.csv("BANK RETURN AND MACRO CAPM.csv", header = TRUE)[,c(2:(cpd+1))];
  nam  = colnames(data);
  tau1 = 0.1;# lower tail
  tau2 = 0.5;# median state
  lambda = c(rep(0.01,cpd - 1),rep(0,nz));
  ws = 424;# the optimal rolling window size

  NetModel = Netden(ws=ws,X=X,Z=Z,nfirm = cpd,tau1=tau1,tau2=tau2,lambda=lambda,n=n,nz=nz,nam=nam,NP=NP)
  ...
  #Step.4. Compare different rolling window sizes
  ws = c(115,230,460);
  for(i in 1:3){
     NetModel = Netden(ws=ws[i],X=X,Z=Z,nfirm = cpd,tau1=tau1,tau2=tau2,lambda=lambda,n=n,nz=nz,nam=nam,NP=NP)
  }
