...
Netden = function(ws,X,Z,nfirm,tau1,tau2,lambda,n,nz,nam,NP){
  betat = list();
  betam = list();
  ...
  for (g in 1:nfirm) {
    ...
    bfunt  = rq_ws_lasso(ws = ws,data = dew,tau = tau1,lambda = lambda,T = n,NP = NP,nfirm = nfirm,nz = nz );
    bfuntm = rq_ws_lasso(ws = ws,data = dew,tau = tau2,lambda = lambda,T = n,NP = NP,nfirm = nfirm,nz = nz);
  ...
  colnames(tot.ct) = nam;
  rownames(tot.ct) = nam;
  write.csv(tot.ct, file = paste0("tot_c_overtime_idio_",ws,".csv"), row.names = FALSE) 
 }
